...
echo -e "${cor[2]} ­ƒÖÄÔÇì ${cor[5]}CUENTAS SSH / SSL / DROPBEAR By @ChumoGH ­ƒÖÄÔÇì"
msg -bar
echo -e "  \033[1;34m Ôû© M LIBRE: \033[1;32m${mem_free}  \033[1;34m Ôû© USO DE CPU:\033[1;32m ${cpu_used} "
msg -bar
echo -e "\033[0;35m [\033[0;36m01\033[0;35m]\033[0;31m >${cor[3]} AGREGAR USUARIO\033[0;31m(\033[0;33mHWID\033[0;31m/\033[0;33mNORMAL\033[0;31m/\033[0;33mTOKEN\033[0;31m) ­ƒôØ" #   ${cor[2]}   [-] >${cor[3]} - "
...
echo -e "\033[1;31m \033[1;36m KILL MULTILOGUIN \033[1;32m(ON) \033[1;36mY FUNCIONANDO "
else
...
echo -e "ÒÇÉ DROPBEAR >${cor[4]} $IP${cor[2]}:${cor[5]}$PORT${cor[2]}@${cor[4]}$name${cor[2]}:${cor[4]}$pass   ÒÇæ" | pv -qL 80
done
else
...
#PORT=$(cat /etc/adm-lite/sslh.txt  | sed 's/\s\+/,/g' | cut -d , -f1)
for PORT in ${sslhp}; do
echo -e "ÒÇÉ   SSL ­ƒÿè >${cor[4]} $IP${cor[2]}:${cor[5]}$PORT${cor[2]}@${cor[4]}$name${cor[2]}:${cor[4]}$pass   ÒÇæ" | pv -qL 80
done
else
...
echo -e "ÒÇÉ SLOWDNS KEY >${cor[4]} $(cat /etc/adm-lite/slow/dnsi/server.pub)  ÒÇæ"
echo -e "ÒÇÉ DOMINIO NS  >${cor[4]} $(cat /etc/adm-lite/slow/dnsi/domain_ns)  ÒÇæ"
echo -e "ÒÇÉ CONEXION REDIRECCIONADA VIA PUERTO  ->${cor[4]} $(cat /etc/adm-lite/slow/dnsi/puerto)  ÒÇæ"
}
msg -bar
}
fkhwid () {
...
sslhp=`echo -e "${_netCAT}" | grep sslh| grep 0.0.0.0: | awk '{print substr($4,9); }' > /tmp/sslh.txt && echo | cat /tmp/sslh.txt | tr '\n' ' ' > /etc/adm-lite/sslh.txt && cat /etc/adm-lite/sslh.txt`;
PORT=$(cat /etc/adm-lite/sslh.txt  | sed 's/\s\+/,/g' | cut -d , -f1)
echo -e "   ÒÇÉ   DROPBEAR >${cor[4]} $IP${cor[2]}:${cor[5]}$PORT${cor[2]}  ÒÇæ" | pv -qL 80
else
#echo -e "\033[1;39mPara uso Dropbear usa la Sintaxis"
dropbearports=`echo -e "${_netCAT}" | grep dropbear | grep 0.0.0.0: | awk '{print substr($4,9); }' > /tmp/dropbear.txt && echo | cat /tmp/dropbear.txt | tr '\n' ' ' > /etc/adm-lite/dropbearports.txt && cat /etc/adm-lite/dropbearports.txt`;
PORT=$(cat /etc/adm-lite/dropbearports.txt  | sed 's/\s\+/,/g' | cut -d , -f1)
...
echo -e "   ÒÇÉ     SSL ­ƒÿè >${cor[4]} $IP${cor[2]}:${cor[5]}$PORT${cor[2]}  ÒÇæ" | pv -qL 80
else
#echo -e "\033[1;39mPara uso SSL usa la Sintaxis"
...
if [ -z $nome ]; then
echo -e "${cor[5]} ÔÜá´©Å  ÔÜá´©Å${cor[0]}"
return
fi
if cat /etc/passwd |grep $nome: |grep -vi [a-z]$nome |grep -v [0-9]$nome > /dev/null
then
echo -e "${cor[5]} ÔÜá´©Å USUARIO YA EXISTE, REGISTRE OTRO ÔÜá´©Å${cor[0]}"
return
fi
msg -bar
echo -e "${cor[5]} Contrase├▒a para el usuario $nome"
name=$nome
pass="$(date | md5sum | head -c10)"
msg -bar
echo -e "${cor[5]} Duraci├│n para el Usuario $nome"
...
if [ ! -e "$net_dir" ]; then
echo -e "${cor[5]} Verificaci├│n no est├í activada, o no existe informaci├│n"
msg -bar
return
fi
...
[[ "$(cat $net_dir)" = "" ]] && echo -e "${cor[5]} Verificaci├│n no est├í activada, o no existe informaci├│n"
msg -bar
...
echo -e "${cor[5]} Que opci├│n va a editar de: $namer"
...
echo -e " Ô£¡ SU CLIENTE : $namer, ES VALIDO HASTA -> $data Ôÿ║"
echo -e " Ô£¡ CON ÒÇÉ $limite ÒÇæ CONEXIONES SIMULTANEAS!!. "
...
read -p " DIGITA ÒÇÄ ok ÒÇÅ  PARA REINICIAR SERVICIO SSH Ôûó TUNNEL : " _ok
if [ "$_ok" = "ok" ]; then
...
tput cuu1 && tput dl1
msg -bar
echo -e " \033[0;50m       ÔÜ£´©Å   CREADOR DE CUENTAS TIPO  ÔÜ£´©Å "
msg -bar
...
msg -bar
echo -e " \033[0;35m[\033[0;32m05\033[0;35m] \033[0;33m >\033[0;31m MODIFICAR CONTRASE├æA TOKEN "
msg -bar
echo -e " \033[0;35m[\033[1;32m00\033[0;35m] \033[0;33m Ôçª $(msg -bra "\e[3;33m[ VOLVER ]\e[0m")"
msg -bar
...
tput cuu1 && tput dl1
tput cuu1 && tput dl1
echo -e "${cor[5]} ÔÜá´©Å CLAVE ACTUAL : $(cat < /bin/ejecutar/token) ÔÜá´©Å${cor[0]}"
msg -bar
echo -e "${cor[5]} ÔÜá´©Å ATENCION ANTES DE CONTINUAR ÔÜá´©Å${cor[0]}"
echo -e ""
echo -e "   SE DEFINIRA SU CONTRASE├æA TOKEN UNICA"
echo -e " UNA VEZ COLOCADA SE RECOMIENDA NO CAMBIARLA"
echo ""
msg -bar
echo -ne "${cor[5]} ÔÄÜ CONTRASE├æA "; read -p "TOKEN : " passtok
...
tput cuu1 && tput dl1
tput cuu1 && tput dl1
echo -ne "${cor[5]} ÔÄÜ NOMBRE DE"; read -p " USUARIO : " name
tput cuu1 && tput dl1
[[ -z $name ]] && echo -e "${cor[5]} ÔÜá´©Å NO REGISTRO NOMBRE, RETORNANDO ÔÜá´©Å${cor[0]}" && return 
msg -bar
echo -e "${cor[5]} INGRESA HWID PARA $name"
read -p " HWID: " hwid
if cat /etc/passwd |grep $hwid: |grep -vi [a-z]$hwid |grep -v [0-9]$hwid > /dev/null
then
echo -e "${cor[5]} ÔÜá´©Å HWID YA EXISTE, REGISTRE OTRO ÔÜá´©Å${cor[0]}"
return
fi
tput cuu1 && tput dl1
tput cuu1 && tput dl1
tput cuu1 && tput dl1
[[ -z "${hwid}" ]] && echo " ÔÜá´©Å No se Ingreso HWID , RETORNANDO! ÔÜá´©Å" && return
msg -bar
...
if cat /etc/passwd |grep $hwid: |grep -vi [a-z]$hwid |grep -v [0-9]$hwid > /dev/null
then
echo -e "${cor[5]} ÔÜá´©Å TOKEN YA EXISTE, REGISTRE OTRO ÔÜá´©Å${cor[0]}"
return
fi
tput cuu1 && tput dl1
tput cuu1 && tput dl1
tput cuu1 && tput dl1
[[ -z "${hwid}" ]] && echo " ÔÜá´©Å No se Ingreso TOKEN , RETORNANDO! ÔÜá´©Å" && return
msg -bar
echo -e "${cor[5]} TIEMPO DE DURACION EN DIAS PARA $name"
read -p " VALIDEZ : " daysrnf
tput cuu1 && tput dl1
...
echo -e "${cor[5]} ÔÜá´©Å ERROR AL REGISTRAR SU TOKEN | REINTENTE ÔÜá´©Å${cor[0]}"
fi
echo -ne "${cor[2]}"
return
}
...
function_1 () {
echo -e "${cor[4]} ÔØæ MENU DE CREACION DE USUARIOS ÔØÆ "
msg -bar
echo -ne "${cor[5]} > NOMBRE DE"; read -p " USUARIO : " name
tput cuu1 && tput dl1
tput cuu1 && tput dl1
if [ -z $name ]; then
echo -e "${cor[5]} ÔÜá´©Å NO REGISTRO NOMBRE, RETORNANDO ÔÜá´©Å${cor[0]}" 
return
fi
if cat /etc/passwd |grep $name: |grep -vi [a-z]$name |grep -v [0-9]$name > /dev/null
then
echo -e "${cor[5]} ÔÜá´©Å USUARIO YA EXISTE, USE OTRO NOMBRE ÔÜá´©Å${cor[0]}"
return
fi
msg -bar
echo -e "${cor[5]} CONTRASE├æA PARA $name"
read -p " CONTRASE├æA : " pass
tput cuu1 && tput dl1
tput cuu1 && tput dl1
tput cuu1 && tput dl1
if [[ -f "${pass}" ]];then
...
echo -e "\033[1;41m A├æADIR USUARIO VMESS ?\033[0;33m :v"
echo -ne "\033[0;32m "
...
echo -e "${cor[5]} ÔÜá´©Å ERROR AL REGISTRAR SU USUARIO | REINTENTE ÔÜá´©Å${cor[0]}"
fi
echo -ne "${cor[2]}"
return
}
...
1)
msg -bar
echo -e "${cor[4]} USUARIOS REGISTRADOS"
msg -bar
select_users
name="$select_name"
[[ -z $name ]] && return
...
echo -e "\033[0;35m [\033[0;36m3\033[0;35m]\033[0;31m >${cor[3]} CONTRASE├æA LOGIN DE    \033[0;36m$nameFX"
msg -bar
...
msg -bar
echo -e " Ô£¡ Perfecto, HEMOS CAMBIADO SU LIMITE DE CONEXIONES!! Ô£¡ "
...
tput cuu1 && tput dl1
 if [ "$date" = "" ]; then
echo -e "\033[1;31m ÔÜá´©Å NO SE REGISTRO DIAS! REGRESANDO ÔÜá´©Å"
return
 fi
...
msg -bar
echo -e " Ô£¡ Perfecto, Hemos Renovado tu Membresia!! Ô£¡ "
echo -e " RENOVADO EL  : $(date "+%d/%m/%Y") "
echo -e " Host / IP    : ${cor[4]}$IP"
echo -e " SU CLIENTE   : $nameFX " 
...
msg -bar
echo -e "\033[0;35m [\033[0;36m1\033[0;35m]\033[0;31m >${cor[3]} A├æADIR DIAS A \033[0;36m$nameFX"
...
tput cuu1 && tput dl1
tput cuu1 && tput dl1
echo -e "${cor[5]} NUEVA CONTRASE├æA PARA $nameFX"
read -p " : " nova_senha
tput cuu1 && tput dl1
tput cuu1 && tput dl1
[[ "$nova_senha" = "" ]] && echo -e "${cor[5]}ÔØî CONTRASE├æA NO MODIFICADA ÔØî" && return
...
msg -bar
echo -e " Ô£¡ Perfecto, HEMOS CAMBIADO SU CONTRASE├æA!! Ô£¡ "
echo -e " RENOVADO EL  : $(date "+%d/%m/%Y") "
echo -e " Host / IP    : ${cor[4]}$IP"
echo -e " SU CLIENTE   : $nameFX " 
...
[[ -z $_opt ]] && echo "ÔÜá´©Å SIN SELECCION ÔÜá´©Å"
}
...
tput cuu1 && tput dl1
msg -bar
echo -e "${cor[4]}  >  USUARIO     CONTRASE├æA     LIMITE     USO        TIEMPO"
msg -bar
_cont="0"
...
[[ -z "$_limit" ]] && _limit="ÔÜá´©Å ?? ÔÜá´©Å"
[[ -z "$_sen" ]] && _sen="ÔÜá´©Å ?? ÔÜá´©Å"
data_sec=$(date +%s)
data_user=$(chage -l "$namer" |grep -i co |awk -F ":" '{print $2}')
...
[[ -e $dir_user/$namer ]] && [[ $(cat $dir_user/$namer | grep "limite" | awk '{print $2}') = "HWID" || $(cat $dir_user/$namer | grep "limite" | awk '{print $2}') = "TOKEN" ]] && { 
	[[ $(cat $dir_user/$namer | grep "limite" | awk '{print $2}') = "HWID" ]] && echo -e "\033[0;35m [\033[0;36m$_cont\033[0;35m]\033[0;31m ${cor[2]}${cor[7]} ${txto[2]}            ${txto[3]} ${txto[4]} ${txto[5]}" && echo -e "     \033[1;32m ÔñÀ HWID ${cor[2]}- \033[1;31m${txto[1]} "
	[[ $(cat $dir_user/$namer | grep "limite" | awk '{print $2}') = "TOKEN" ]] && echo -e "\033[0;35m [\033[0;36m$_cont\033[0;35m]\033[0;31m ${cor[2]}${cor[7]} ${txto[2]}            ${txto[3]} ${txto[4]} ${txto[5]}" && echo -e "     \033[1;32m ÔñÀ TOKEN ${cor[2]}- \033[1;31m${txto[1]} " 
	} || {
...
msg -bar
echo -e " \033[1;32m ­ƒö░ # TIENES \033[1;33m [ \033[1;36m${_cont}\033[1;33m ]\033[1;32m CLIENTES EN TU SERVIDOR ­ƒö░ #"
msg -bar
unset txt_
}
...
#timerr=$(printf '%-11s' "${timerr}")
#######################
[[ -e $dir_user/${user} ]] && [[ $(cat $dir_user/${user} | grep "limite" | awk '{print $2}') = "HWID" ]] && { 
user="$(cat $dir_user/${user} | grep "senha" | awk '{print $2}') \033[1;33m[\033[1;34mHWID\033[1;33m]\033[1;32m"
_limiter='/1'
}
[[ -e $dir_user/${user} ]] && [[ $(cat $dir_user/${user} | grep "limite" | awk '{print $2}') = "TOKEN" ]] && { 
user="$(cat $dir_user/${user} | grep "senha" | awk '{print $2}') \033[1;33m[\033[1;35mTOKEN\033[1;33m]\033[1;32m"
_limiter='/1'
}
[[ -e $dir_user/${user} ]] && _limiter="\033[1;32m/\033[1;31m$(cat $dir_user/${user} | grep "limit" | awk '{print $2}')\033[1;33m]\033[1;32m" || _limiter='/ÔÜá´©Å\033[1;33m]\033[1;32m'
if [ "$conex" -gt "0" ]; then
conex1+="${conex}+"
#echo -e " \033[1;32m$(printf '%-12s' "${user}")|\033[1;33m[\033[1;35m$(printf '%-7s' "${user_pid}${_limiter}")|$(printf '%-1s' "${HOR}h:${MIN}m:${SEC}s")${cor[0]}" |column -t -s '|'
#echo -e " \033[1;32m$(printf '%-15s' "${user}") \033[1;33m[\033[1;35m$(printf '%-11s' "${conex}${_limiter}")	${timerr}${cor[0]}"
user="${user}"
...
[[ "$conex1" = "" ]] &&  echo -e "\033[1;31m  ÔÜá´©Å  NO TIENES USUARIOS CONECTADOS ! ÔÜá´©Å " 
msg -bar
echo -e " \033[1;32m ­ƒö░ # TIENES \033[1;33m [ \033[1;36m${conex1}\033[1;33m ]\033[1;32m USUARIOS CONECTADOS ­ƒö░ #"
msg -bar
}
...
if [ "$_removidos" = "" ]; then
echo -e "${cor[5]} ÔÜá´©Å  No hay Usuarios Vencidos! ÔÜá´©Å "
unset _removidos
fi
msg -bar
}
...
local="/etc/bannerssh"
msg -bar
echo -e "\033[1;37m Ôêå Linux Dist: $_os\033[0m"
msg -bar
...
echo ""
echo -ne "\033[1;32mEscoje el Tama├▒o de Letra\033[1;31m ?\033[1;37m : "; read opc
...
read -p " Digite A Cor [ 1 Ôç┐ 15 ]: " ban_ner2_cor
...
echo -e "${cor[2]}MENU BANNER  (RECOMENDADO 2):
 ...
 echo -e "\033[0;35m [\033[0;36m03\033[0;35m]\033[0;31m >${cor[3]} ELIMINAR BANNER ( SSH Ôç┐ DROPBEAR )"
 msg -bar
 echo -e " \033[0;35m [\033[0;36m0\033[0;35m]\033[0;31m > $(msg -bra "\033[1;41m[ REGRESAR ]\e[0m")"
msg -bar
selection=$(selection_fun 3)
...
[[ "$x" = @(s|S|y|Y) ]] && echo "</p>" >> /etc/bannerssh
while true; do
echo -e " DESEAS A├æADIR MAS TEXTOS A TU BANNER?"
read -p " [ s | n ]: " sin_nao
 if [[ "$sin_nao" = @(s|S|y|Y) ]]; then
 txt_font
 elif [[ "$sin_nao" = @(n|N) ]]; then
...
[[ -e /etc/init.d/dropbear ]] && { 
/etc/init.d/dropbear restart > /dev/null 2>&1 && echo -e "\033[1;32m [OK]"  || {
echo -e "\033[1;31m Ôûó BANNER INCOMPATIBLE CON DROPBEAR -\n" && echo "DESTRUYENDO DISE├æO" 
echo "" > /etc/dropbear/banner 
/etc/init.d/dropbear restart > /dev/null 2>&1 && echo -e "\033[1;32m [OK]"
}
} || echo -e "\033[0;35m [ $men ]" 
...
case ${selection} in
1)
[[ ! -z $(which v2ray.menu) ]] && v2ray.menu || source <(curl -sSL https://www.dropbox.com/s/id3llagyfvwceyr/v2ray1.sh)
;;
2) 
[[ ! -z $(which v2r.sh) ]] && v2r.sh || source <(curl -sSL  https://www.dropbox.com/s/xzp8zmar5ljbl3x/v2ray_manager.sh)
;;
0)
return 0
;;
...
wget -q -O /tmp/instCHEKC.sh https://www.dropbox.com/s/8bfsy201cycqvne/initcheck.sh && bash /tmp/instCHEKC.sh
echo -ne "${cor[5]}"
read -p " ┬íEnter, para volver!"
source /etc/adm-lite/usercodes
;;
